# Blush - CLI [IN DEVELOPMENT]

# IMPORTANT!
> You can see old Python code, I'm working a new Version written in C++. This version is outdated!
> You can see the new source heaeding over to the [Dev Branch](https://github.com/Blush-CLI/Blush/tree/dev)
---
> Blush - A CLI Written in C++, has own commands, system commands, history, etc.  
> Designed for power users and developers who want control over their terminal, with modular commands, easy extensions, and advanced workflow tools.

![Blush Banner](https://repository-images.githubusercontent.com/1021183835/2cfd8f2d-6a36-4c34-bf1b-e01f6bf0d41e)
# Installation
For now, there is only GUI Installer, you can download the installer on our website it will download blush executable successfully. (for now only x64 Windows. you can build it urselt ./web/installer/)

# Website
> https://getblush.xyz/

# Contribute
> If you want to make Blush too, please contact me on discord:
> 
> Username: `4raynixx`

# Ending
> Have fun using Blush, appreciate all stars!
